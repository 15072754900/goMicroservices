...
func (l *CreateHomestayOrderLogic) CreateHomestayOrder(in *pb.CreateHomestayOrderReq) (*pb.CreateHomestayOrderResp, error) {
	...
	liveDays := int64(order.LiveEndDate.Sub(order.LiveStartDate).Seconds() / 86400) // sum days in total
	...
	if err != nil {
		return nil, errors.Wrapf(xerr.NewErrCode(xerr.DB_ERROR), "Order Datanase Exception order : %+v, err: %v", order, err)
	}
