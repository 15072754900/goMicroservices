...
var (
	...
	cacheLookCpHomestayActivityIdPrefix = "cache:lookCp:homestayActivity:id:"
)
...
func (m *defaultHomestayActivityModel) withSession(session sqlx.Session) *defaultHomestayActivityModel {
	return &defaultHomestayActivityModel{
...
func (m *defaultHomestayActivityModel) Delete(ctx context.Context, id int64) error {
	lookCpHomestayActivityIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayActivityIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, lookCpHomestayActivityIdKey)
	return err
}
...
func (m *defaultHomestayActivityModel) FindOne(ctx context.Context, id int64) (*HomestayActivity, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, lookCpHomestayActivityIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayActivityModel) Insert(ctx context.Context, data *HomestayActivity) (sql.Result, error) {
	lookCpHomestayActivityIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayActivityIdPrefix, data.Id)
...
func (m *defaultHomestayActivityModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLookCpHomestayActivityIdPrefix, primary)
}
