...
var (
	...
	cacheLookCpHomestayBusinessIdPrefix     = "cache:lookCp:homestayBusiness:id:"
	cacheLookCpHomestayBusinessUserIdPrefix = "cache:lookCp:homestayBusiness:userId:"
)
...
func (m *defaultHomestayBusinessModel) withSession(session sqlx.Session) *defaultHomestayBusinessModel {
	return &defaultHomestayBusinessModel{
...
func (m *defaultHomestayBusinessModel) Delete(ctx context.Context, id int64) error {
	...
	lookCpHomestayBusinessIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessIdPrefix, id)
	lookCpHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessUserIdPrefix, data.UserId)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, lookCpHomestayBusinessIdKey, lookCpHomestayBusinessUserIdKey)
	return err
}
...
func (m *defaultHomestayBusinessModel) FindOne(ctx context.Context, id int64) (*HomestayBusiness, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, lookCpHomestayBusinessIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayBusinessModel) FindOneByUserId(ctx context.Context, userId int64) (*HomestayBusiness, error) {
	lookCpHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessUserIdPrefix, userId)
	var resp HomestayBusiness
	err := m.QueryRowIndexCtx(ctx, &resp, lookCpHomestayBusinessUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultHomestayBusinessModel) Insert(ctx context.Context, data *HomestayBusiness) (sql.Result, error) {
	lookCpHomestayBusinessIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessIdPrefix, data.Id)
	lookCpHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessUserIdPrefix, data.UserId)
...
func (m *defaultHomestayBusinessModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLookCpHomestayBusinessIdPrefix, primary)
}
