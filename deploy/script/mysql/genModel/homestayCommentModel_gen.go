...
var (
	...
	cacheLookCpHomestayCommentIdPrefix = "cache:lookCp:homestayComment:id:"
)
...
func (m *defaultHomestayCommentModel) withSession(session sqlx.Session) *defaultHomestayCommentModel {
	return &defaultHomestayCommentModel{
...
func (m *defaultHomestayCommentModel) Delete(ctx context.Context, id int64) error {
	lookCpHomestayCommentIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayCommentIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, lookCpHomestayCommentIdKey)
	return err
}
...
func (m *defaultHomestayCommentModel) FindOne(ctx context.Context, id int64) (*HomestayComment, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, lookCpHomestayCommentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayCommentModel) Insert(ctx context.Context, data *HomestayComment) (sql.Result, error) {
	lookCpHomestayCommentIdKey := fmt.Sprintf("%s%v", cacheLookCpHomestayCommentIdPrefix, data.Id)
...
func (m *defaultHomestayCommentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLookCpHomestayCommentIdPrefix, primary)
}
