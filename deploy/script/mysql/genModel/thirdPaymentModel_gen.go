...
var (
	...
	cacheLookCpThirdPaymentIdPrefix = "cache:lookCp:thirdPayment:id:"
	cacheLookCpThirdPaymentSnPrefix = "cache:lookCp:thirdPayment:sn:"
)
...
func (m *defaultThirdPaymentModel) withSession(session sqlx.Session) *defaultThirdPaymentModel {
	return &defaultThirdPaymentModel{
...
func (m *defaultThirdPaymentModel) Delete(ctx context.Context, id int64) error {
	...
	lookCpThirdPaymentIdKey := fmt.Sprintf("%s%v", cacheLookCpThirdPaymentIdPrefix, id)
	lookCpThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheLookCpThirdPaymentSnPrefix, data.Sn)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, lookCpThirdPaymentIdKey, lookCpThirdPaymentSnKey)
	return err
}
...
func (m *defaultThirdPaymentModel) FindOne(ctx context.Context, id int64) (*ThirdPayment, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, lookCpThirdPaymentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultThirdPaymentModel) FindOneBySn(ctx context.Context, sn string) (*ThirdPayment, error) {
	lookCpThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheLookCpThirdPaymentSnPrefix, sn)
	var resp ThirdPayment
	err := m.QueryRowIndexCtx(ctx, &resp, lookCpThirdPaymentSnKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultThirdPaymentModel) Insert(ctx context.Context, data *ThirdPayment) (sql.Result, error) {
	lookCpThirdPaymentIdKey := fmt.Sprintf("%s%v", cacheLookCpThirdPaymentIdPrefix, data.Id)
	lookCpThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheLookCpThirdPaymentSnPrefix, data.Sn)
...
func (m *defaultThirdPaymentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLookCpThirdPaymentIdPrefix, primary)
}
